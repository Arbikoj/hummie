...
const HistoricalGraph: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const path = filter === 'daily' ? `data/${year}/${month}/${day}` : `data/${year}/${month}`;
            ...
            if (rawData) {
                if (filter === 'daily') {
                    ...
                    Object.keys(rawData).forEach((day) => {
                        ...
                        const averageTemp =
                            Object.values(dayData).reduce((acc: number, entry: any) => acc + entry.temperature, 0) / Object.keys(dayData).length;
                        const averageHumidity =
                            Object.values(dayData).reduce((acc: number, entry: any) => acc + entry.humidity, 0) / Object.keys(dayData).length;
                        formattedData.push({
                            time: day,
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        ...
                        >
                            ...
                            {selectedDate ? format(selectedDate, filter === 'daily' ? 'PPP' : 'MMMM yyyy') : 'Pick a date'}
                        </Button>
                    </PopoverTrigger>
